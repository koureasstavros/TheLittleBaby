...
- **Forward Pass** – Adds complexity and depth to the model’s understanding.

- **Backward Pass** – Refines the model’s learning by adjusting weights based on errors, guiding it toward better predictions.
...
- **Underfitting** - When the training loss is not low and the validation loss is not low this is a sign of underfitting as the model has not leared enough the seen dataset and cannot generalize to unseen dataset too.

    - train_loss > 1 and val_loss > 1 is bad, the model cannot syntactically complete sentences

- **Overfitting** - When the training loss is to low but the validation loss is high this is a sign of overfitting as the model has leared better the seen dataset and cannot generalize to unseen dataset.

    - train_loss < 1 and val_loss > 1 is bad, the model might syntactically complete sentences

- **Optimalfitting** - When the training loss is low and the validation loss low this is a sign of optimalfitting as the model has leared the seen dataset and can generalize to unseen dataset too. Here are some examples of validation loss:

    - train_loss ~ 1 and val_loss ~ 1 is ok, the model can syntactically complete sentences
    - train_loss < 1 and val_loss < 1 is good, the model can syntactically complete correct sentences

- **Grad** - The .grad attribute of a tensor is where the gradient is stored after backpropagation. It tells you how much a change in that tensor would affect the final output (usually the loss). Think of it as the sensitivity of the loss to that tensor.During training, we want to minimize the loss. To do that, we need to know:
